...
def assign_prs(df):
    ...
    df["PR"] = df.apply(lambda r: (
        "🏅" if ((r["Actual Weight (kg)"]>0 and r["Actual Weight (kg)"]==prs_w.get(r["Exercise"],0)) or
                  (r["Actual Weight (kg)"]==0 and r["Reps"]==prs_r.get(r["Exercise"],-1))) else ""), axis=1)
    return df
...
with st.expander("Weekly summary (last 4 weeks)"):
    ...
    st.download_button("Download weekly CSV", weekly_summary.to_csv(index=False).encode(), "weekly_summary.csv")
